...
interface RawGrowth {
  topic?: string;
  sayBetter?: Array<{ original?: string; upgraded?: string; improved?: string; note?: string }>;
  newExpressions?: Array<{ phrase?: string; meaning?: string; example?: string }>;
  talkMore?: Array<{ angle?: string; starter?: string }>;
}
...
export interface ProcessedGrowth {
  topic: string;
  sayBetter: Array<{ original: string; upgraded: string; note: string }>;
  newExpressions: Array<{ phrase: string; meaning: string; example: string }>;
  talkMore: Array<{ angle: string; starter: string }>;
}
...
export function cleanTranscriptForReport(transcript: string): string {
...
- sayBetter (2-3 items): pick sentences the user actually said that are CORRECT but plain, short, or repetitive, and upgrade them to a richer, more natural version one notch above the user's level (i+1). Do NOT reuse sentences already listed in corrections. The upgrade should demonstrate something learnable: a connector, a vivid verb, added detail, a native chunk.
- newExpressions (3-5 items): teach spoken chunks, collocations, or sentence patterns directly useful for THIS topic and appropriate for the user's level. Prefer high-frequency spoken English over fancy written words. Each needs one natural example sentence in the context of this conversation.
...
function normalizeGrowth(raw: RawReport): ProcessedGrowth | undefined {
